...
int is_valid_csv(FILE *contacts_file)
{
	...
	*/
	...
	if (!strcmp("name", to_lower(strtok(columns, ","))))
		return 0;
	if (!strcmp("number", to_lower(strtok(NULL, ","))))
		return 0;
	if (!strcmp("address", to_lower(strtok(NULL, ","))))
		return 0;
	return 1;
}
...
bool add_contact(char *name, char *number, char *address)
{
	...
	{
		load_data(name, number, address);
		return true;
	}
	return false;
}

int export_contacts(FILE *contacts_file)
{
	/*
...
int balance_check(char *number, int amount)
{
	/*
